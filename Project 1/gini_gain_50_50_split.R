...
set.seed(90)
create_train_test <- function(bankData, size = 0.5, train = TRUE) {
  n_row = nrow(bankData)
...
data_train <- create_train_test(bankData, 0.5, train = TRUE)
data_test <- create_train_test(bankData, 0.5, train = FALSE)
library(rpart)
info_dt <- rpart(y~., data = data_train, method = 'class', parms=list(split='gini'))
library(rpart.plot)
